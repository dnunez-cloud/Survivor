...
using UnityEngine;
//componente del elemento arma, police 9mm. Controla las balas, sonido, modo disparo y zoom en cámara de disparo(modo ADS). 
//Este script tiene todo el funcionamiento del arma, el raycast del disparo y las animaciones.
public enum ModoDeDisparo1
{
    SemiAuto,
    FullAuto
}
public class LogcaArma2 : MonoBehaviour
{
    ...
    public MoverSinGravedad2 moversingravedad;          //crea variable para asignarle el componente del jugador
    ...
    public ModoDeDisparo1 modoDeDisparo = ModoDeDisparo1.FullAuto;
    ...
    void Awake()
    {
        rb = GameObject.Find("Jugador2").GetComponent<Rigidbody>();         //asigna en la variable el componente
    }
    ...
    void Update()
    {
        if (modoDeDisparo == ModoDeDisparo1.FullAuto && Input.GetButton("Fire2"))          //controla si el jugador está presionando el botón de disparo
        {
            RevisarDisparo();
        }
        else if (modoDeDisparo == ModoDeDisparo1.SemiAuto && Input.GetButtonDown("Fire2"))   //controla si el jugador está presionando el botón de disparo
        {
            RevisarDisparo();
        }
        if (Input.GetButtonDown("Reload1"))                       //controla si el jugador presiona el botón para recargar
        {
            RevisarRecargar();
        }
        if (Input.GetButtonDown("ADS"))                     //controla si el jugador presiona el botón para modo ADS
        {
        ...
        if (Input.GetButtonUp("ADS"))                     //controla si el jugador dejó de presionar el botón para modo ADS
        {
            estaADS = false;
        }
        ...
        singravedad = GameObject.Find("Jugador2").GetComponent<MoverSinGravedad2>().singravedad;       //asigna en la variable el componente
    }
